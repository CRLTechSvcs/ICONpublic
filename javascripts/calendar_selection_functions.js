...
function displayTitleInfo(results, textStatus) {
	if ((results.search(/Error/i) != -1) || (results.search(/\<b>Notice/i) != -1 )) {
		...
		if (data.pubTitle == "") {
			$('#sidebar').prepend('<div class="matchingColumn"><h4>Title Information</h4></div><div class="not_icon_title" id="title_info"><p>This title does not appear in ICON.</p></div>');
			...
			if (tempRefDataURI != '' && tempRefDataURI != null) {
				...
				titleInfoDisplayString += '<div id="add_ref">' + addRefString + '</div>';
			}
			//$('#sidebar').prepend('<div><h4>Title Information</h4></div><div id="title_info"><p>' + titleInfoDisplayString + '</p></div>');
			document.getElementsByClassName('panel')[0].style.paddingTop = "0px";
			document.getElementsByClassName('panel')[0].style.paddingBottom = "0px";
			document.getElementsByClassName('panel')[0].innerHTML = '<div id="title_info" style="border: none; font-size: .85em;"><p>' + titleInfoDisplayString + '</p></div>';
		...
		$.get("get_format_list.php", { 'selectedPubId': selectedPubId }, displayFormatList);
		$.get("get_repository_list.php", { 'selectedPubId': selectedPubId }, displayRepositoryList);
	}
}
function displayFormatList(results, textStatus) {
	if ((results.search(/Error/i) != -1) || (results.search(/\<b>Notice/i) != -1 )) {
		document.writeln(results);
	} else {
		var availableFormatsString = '<div id="available_formats" class="pub_data_content"><ul>';
		//prompt("calendar_selection_functions.js displayFormatList gets results\n" +results, results);
		var data = JSON.parse(results);
		var formats = data.formatList;
		if (formats.length > 0) {
			for (var formatName in formats) {
				//availableFormatsString += formats[formatName] + '<br />';
				availableFormatsString += '<li>' + formats[formatName] + '</li>';
			}
			availableFormatsString += '</ul></div>';

				//AJE 2012-11-27: moved formats from #sidebar to #page: next puts the formats in #sidebar
				//$('<div><h4>Available Formats</h4></div><div id="available_formats"><p></p>' + availableFormatsString + '<p></p></div>').insertBefore($('h4:contains("Genealogy of paper")')[0]);

				// AJE 2012-11-27: moved repositories from #sidebar to #page
...
function displayRepositoryList(results, textStatus) {
	if ((results.search(/Error/i) != -1) || (results.search(/\<b>Notice/i) != -1 )) {
		document.writeln(results);
	} else {

		var availableRepositoriesString = '<div id="available_repositories" class="pub_data_content"><ul>';

		var tempRepository = tempRepositoryName = tempMinYear = tempMaxYear = null;
		var data = JSON.parse(results);
		var repositories = data.repositories;
			if (repositories.length > 0) {
				for (var i = 0; i < repositories.length; i++) {
				tempRepository = repositories[i].repositoryInfo;
				tempRepositoryName = tempRepository.repositoryName;
				tempMinYear = tempRepository.minYear;
				tempMaxYear = tempRepository.maxYear;
				if (tempMinYear != tempMaxYear) {
					availableRepositoriesString += '<li>' + tempRepositoryName + ' [' + tempMinYear + '-' + tempMaxYear + ']</li>';
				} else {
					availableRepositoriesString += '<li>' + tempRepositoryName + ' [' + tempMinYear + ']</li>';
				}
			}
			availableRepositoriesString += '</ul></div>';

				//AJE 2012-11-27: moved repositories from #sidebar to #page: next puts repositories in #sidebar
				//$(availableRepositoriesString).insertBefore($('h4:contains("Genealogy of paper")')[0]);

				// AJE 2012-11-27: moved repositories from #sidebar to #page
			$('#repositories_list').html( availableRepositoriesString );
			//resetHeightsAndOffsets($('.matchingColumn')); //headTagContents.php // 2013-02-25 20:18:54 not in JHunt new version
		}
	}
}
...
function displayDecadesBarchart() {
	...
	//Hide 1690s decade on timeline unless there are issues in that range
	var firstYear = issueCounts[0].year;
	if (firstYear.substr(0,3) == '169') {
		$('#li_1690s').css('display', 'block');
		$('#li_post_1790s').css('display', 'none');
	} else {
		$('#li_1690s').css('display', 'none');
		$('#li_post_1790s').css('display', 'block');
	}
	for (var i in issueCounts) {
		tempYear = issueCounts[i].year;
		tempIssueCount = issueCounts[i].count;
		tempBarSize = (tempIssueCount/maxIssueCount * 24);
		//Maximum bar height is 24px
...
function displayFilterLinks() {
	if (availableDays.length == 0) {
		...
		$('#tempContainer').append('<br /><br /><div>Filter by:</div><div><a id="Filter by Repository blah" href="javascript:;">Repository</a></div>');
...
function displayRepositoryLinks() {
	...
	$('#inner_issue_info_display').append('<div><br /><a class="breadcrumb" href="" onclick="displayFilterLinks(); return false;">Back to filter list</a></div>');
...
function displayFormatLinks() {
	...
	$('#inner_issue_info_display').html('');
	//Create a 'Back' link to redisplay filter list
	$('#inner_issue_info_display').append('<br /><div><a class="breadcrumb" href="javascript:;" onclick="displayFilterLinks(); return false;">Back to filter list</a></div>');
	//Create repository links
...
function buildRepositoryIssueInfoString(issueIndex) {
	...
	*/
	...
	issueInfoDisplayString +=   '<br /><span class="issue_info_label">REPOSITORY: &nbsp;</span><span class="issue_info_text">' + tempRepositoryName + '</span><br />';
	if (tempPhysCondition == "condition unspecified/unknown") {
		issueInfoDisplayString += '<span class="issue_info_label">CONDITION: &nbsp;</span><span class="issue_info_text">Unspecified or unknown</span><br />';
	} else {
		issueInfoDisplayString += '<span class="issue_info_label">CONDITION: &nbsp;</span><span class="issue_info_text">' + tempPhysCondition + '</span><br />';
	}
	issueInfoDisplayString += '<span class="issue_info_label">FORMAT: &nbsp;</span><span class="issue_info_text">' + tempFormat + '</span><br />';
	if (tempArchiveStatus == "archive status unspecified/unknown") {
		issueInfoDisplayString += '<span class="issue_info_label">ARCHIVE STATUS: &nbsp;</span><span class="issue_info_text">Unspecified or unknown</span><br />';
	} else {
		issueInfoDisplayString += '<span class="issue_info_label">ARCHIVE STATUS: &nbsp;</span><span class="issue_info_text">' + tempArchiveStatus + '</span><br />';
	}
	//6-15-12 CFJ - per Amy, do not display issue notes
...
function buildFormatIssueInfoString(issueIndex) {
	...
	*/
	tempUpdateDate = availableIssues[issueIndex].updateDate;
	tempNotes = availableIssues[issueIndex].notes;
	issueInfoDisplayString +=   '<br /><span class="issue_info_label">REPOSITORY: &nbsp;</span><a href="javascript:;" onclick="displayRepositoryInfoFromFormat(' + issueIndex + ');">' + tempRepositoryName + '</a><br />';
	if (tempPhysCondition == "condition unspecified/unknown") {
		issueInfoDisplayString += '<span class="issue_info_label">CONDITION: &nbsp;</span><span class="issue_info_text">Unspecified or unknown</span><br />';
	} else {
		issueInfoDisplayString += '<span class="issue_info_label">CONDITION: &nbsp;</span><span class="issue_info_text">' + tempPhysCondition + '</span><br />';
	...
	//6-19-12 CFJ - per Amy, do not display issue note field
	//if (tempNotes != '' && tempNotes != null) {
	//	issueInfoDisplayString +=	'<span class="issue_info_label">NOTES: &nbsp;</span><span class="issue_info_text">' + tempNotes + '</span><br />';
	//}
	if (tempUpdateDate != '0000-00-00' && tempUpdateDate != null) {
		tempUpdateDate = toTitleCase(monthAbbrevArray[parseInt(tempUpdateDate.substr(5,2),10)-1])+ " " + parseInt((tempUpdateDate.substr(8,2)),10) + ", " + tempUpdateDate.substr(0,4);
		issueInfoDisplayString +=	'<span class="issue_info_label">LAST UPDATED: &nbsp;</span>' + tempUpdateDate + '<br />';
...
function displayRepositoryInfo(results, textStatus) {
	...
	if ((results.search(/Error/i) != -1) || (results.search(/\<b>Notice/i) != -1 )) {
		...
		if (tempHomePage != null && tempHomePage != '') {
			reposInfoDisplayString = '<a href="' + tempHomePage + '" target="_blank"> ' + tempReposName + '</a><br />';
			//6-19-12 CFJ: Amy prefers to see repository name only, not ID.  Keeping alternate statements in case this requirement changes
			//reposInfoDisplayString = '<a href="' + tempHomePage + '" target="_blank"> ' + tempReposName + ' (' + tempReposId + ')</a><br />';
		} else if (tempOpac != null && tempOpac != '') {
			reposInfoDisplayString = '<a href="' + tempOpac + '" target="_blank">' + tempReposName + '</a><br />';
			//reposInfoDisplayString = '<a href="' + tempOpac + '" target="_blank">' + tempReposName + ' (' + tempReposId + ')</a><br />';
		} else {
			reposInfoDisplayString = tempReposName + '<br />'
			//reposInfoDisplayString = tempReposName + ' (' + tempReposId + ')<br />'
		}
		reposInfoDisplayString += reposAddressString;

		/*repositories.repos_note is filled in only 235 cases;
		these are usually notes like 'aka 0MQ, MQV' [for repos_id ZAB].
		These tell us other codes used by same institution and are only
		internally useful at this time. Display of repos_note is commented
		out until we have a need to show.--AJE

		if (tempReposNote != null && tempReposNote != '') {
			reposInfoDisplayString += '<div class="repos_note">' + tempReposNote + '</div>';
		}
		*/
		$('<h4>Repository Information</h4><div id="repository_info"><p>' + reposInfoDisplayString + '</p></div>').insertAfter('#title_info');
