...
function validateSearchTerms() {
	...
	if ($('#state_country_dropdown').val() != "0") {
		searchterm_entered = true;
	} else {
		$('.searchbox').each(function( index ) {
	...
	if (!searchterm_entered) {
		...
		var search_title = search_lccn = search_oclc = search_issn = search_city = search_keyword = search_from_year = search_to_year = search_state_country = search_state_country_name = '';
		...
		if ($('#search_type_link').text().search('advanced') == -1) {
			//Advanced search
...
function displayTitles(results, textStatus) {
	var pubId = pubTitle = pubCity = countryId = countryName = pubBgnDate = pubEndDate = tempHREF = null;
	...
	if (results == "No results returned") {
		...
		for (var i = 0; i < titleArray.length; i++) {
		  //alert("in the for loop displayTitles");
			...
			if (pubBgnDate == pubEndDate) {
				titleInfo += ': published ' + pubBgnDate + ')';
			} else {
				titleInfo += ': published ' + pubBgnDate + '-' + pubEndDate + ')';
			}
