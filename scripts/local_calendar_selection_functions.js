...
function displayTitleInfo(results, textStatus) {
	if ((results.search(/Error/i) != -1) || (results.search(/\<b>Notice/i) != -1 )) {
		...
		if (data.pubTitle == "") {
			$('#title_date_display').prepend('<div><h4>Title Information</h4></div><div id="title_info"><p>This title does not appear in ICON.</p></div>');
			...
			$('#title_date_display').prepend('<div><h4>Title Information</h4></div><div id="title_info"><p>' + titleInfoDisplayString + '</p></div>');
			$('#loading-calendar').html('<p>Searching for issues&nbsp; . . . &nbsp;&nbsp;<img src="images/loading-calendar.gif" /></p>');
...
function displayRepositoryInfo(results, textStatus) {
	...
	if ((results.search(/Error/i) != -1) || (results.search(/\<b>Notice/i) != -1 )) {
		...
		$('#title_date_display').append('<div id="repository_info"><h4>Repository Information</h4><p>' + reposInfoDisplayString + '</p></div>');
	}
}
