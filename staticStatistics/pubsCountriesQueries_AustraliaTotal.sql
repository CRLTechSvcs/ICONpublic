...
SELECT /* only valid issues */
	CONCAT("All Australia for countryInfo[issue_dates]: ", COUNT(i.issue_id)) AS numIssues,
...
WHERE (
	...
	AND
	(p.country_id LIKE '__a' AND p.country_id <> 'ncc') /* code 'ncc' is an area */
)
...
SELECT /* incl. invalid issues */
	CONCAT("All Australia for countryInfo[country_issues]: ", COUNT(i.issue_id)) AS numIssuesWholeCountry,
	MIN(i.issue_date) AS minDate,
...
SELECT /* ignore issues */
	CONCAT("All Australia for countryInfo[country_pubs]: ", "...") AS numIssuesWholeCountry,
